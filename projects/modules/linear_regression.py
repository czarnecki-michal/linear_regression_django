...
class LinearRegression:
    ...
    def fit(self, x, y):
        ...
        if len(x) == len(y):
            ...
            raise ValueError(
                "Wymiary argumentów musza byc takie same. Podano ({}) i ({})".format(len(x), len(y))
                )
    ...
    def predict(self, x):
        ...
        try:
            ...
            print("Wartosc x musi byc lista")
            raise
    ...
    def calculate_error(self, y, y_pred):
        ...
        if len(y) == len(y_pred):
            ...
            raise ValueError("Wymiary argumentów musza byc takie same. Podano ({}) i ({})".format(len(y), len(y_pred)))


    def print_results(self, x, y):
        print("Współczynnik kierunkowy: {}\nWyraz wolny: {} \nWspółczynnik determinacji R^2: {}"
              .format(round(self.coef[0], 5), round(self.coef[1], 5), round(self.r_squared, 5)))
